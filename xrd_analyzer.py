...
# ============================================================================
# SCIENTIFIC PEAK DETECTION WITH PHYSICAL VALIDATION
# ============================================================================
def detect_peaks_with_validation(two_theta, intensity, background,
                                 min_distance_deg=1.0, min_prominence=0.03):
    """
...
def calculate_d_spacing(theta_deg, wavelength=1.5406):
    """
    Calculate d-spacing from Bragg's law: nλ = 2d sinθ
    """
...
def williamson_hall_analysis(peaks, wavelength=1.5406, instrument_fwhm_rad=0.0):
    ...
    for p in peaks:
        ...
        # Upper bound relaxed to 5.0° for nanocrystalline materials
        # (Scherrer broadening can exceed 2° for crystallites < 5 nm).
    ...
    for p in valid_peaks:
        ...
        x_vals.append(4 * np.sin(theta_rad))
    ...
    try:
        ...
        K = 0.9
        size_nm = (K * wavelength) / (intercept * 10)
        microstrain = slope / 4
...
class AdvancedXRDAnalyzer:
    ...
    def validate_crystallographic_results(self, xrd_results: Dict) -> Dict:
        ...
        if ci < 0 or ci > 1:
            ...
            validation['warnings'].append("Crystallinity index = 1.00 is physically impossible")
        ...
        for peak in peaks:
            ...
            if fwhm < 0.01 or fwhm > 5.0:
                validation['warnings'].append(
                    f"Unphysical FWHM: {fwhm:.4f}° at 2θ={peak.get('position', 0):.2f}°"
                )
        ...
        validation['scientific_checks'] = [
            ...
            },
            {
                ...
                'value': f"{np.mean([p.get('fwhm_deg', 0) for p in peaks]):.3f}° avg" if peaks else "N/A",
                'expected': '0.03-3.0°'
            }
        ]
    ...
    def calculate_nano_tolerance(self, size_nm):
        """
        Physics-based tolerance for nanocrystalline materials.
        """
        if size_nm < 5:
            return 0.10
        elif size_nm < 10:
    ...
    def complete_analysis(self, two_theta, intensity, elements=None):
        ...
        try:
            ...
            # CRYSTALLITE SIZE ANALYSIS
            # ----------------------------------------------------------
            mean_size = 0.0  # safe default to prevent NameError later
            if validated_peaks:
