...
- Full lattice parameters (a,b,c,α,β,γ), density, space group, point """
            ...
            with MPRester(self.mp_api_key) as mpr:
                ...
                for doc in results[:max_results]:
                    if doc.structure:
                        ...
                        # Store the structure directly in the candidate
                        structures.append({
                            'database': 'MaterialsProject',
                            'id': doc.material_id,
                            'formula': doc.formula_pretty,
                            'space_group': doc.symmetry.get('symbol', 'Unknown') if doc.symmetry else 'Unknown',
                            'structure': structure,  # Direct structure object – no download needed
...
def simulate_from_structure(structure, wavelength):
    """Simulate XRD directly from a structure object."""
    try:
...
def simulate_from_cif(cif_url, wavelength, formula_hint=""):
    ...
    If candidate has a 'structure' key, use that directly (no download).
    """
    if not PMG_AVAILABLE:
        return np.array([]), np.array([]), [], []

    # If we already have a structure (from Materials Project), use it directly
    # Note: This function is called with a candidate that may have a 'structure' key,
    # but we don't have access to the candidate here. We need to pass the structure separately.
    # Instead, we'll handle direct structure simulation in the main loop.
    # For now, keep HTTP download.

    # (The main loop will handle structure simulation separately)

    # HTTP download for non-MP entries
    user_agents = [
...
# ============================================================================
# FALLBACK DATABASE (shortened for brevity – include full list from version 8)
# ============================================================================
FALLBACK = [
    ...
    # ... (include all entries from version 8)
]
...
def identify_phases_universal(two_theta=None, intensity=None, wavelength=1.5406,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
        ...
        for struct in candidates:
            # If it's a Materials Project candidate with a structure, simulate directly
    ...
    # STEP 5: Built-in library if no matches
    if not results:
